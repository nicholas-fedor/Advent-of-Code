...
## Analysis
$$
Overall Sum = Game \sum_{n=1}^{N} ({Blue}_{MaxSampleValue} \times {Red}_{MaxSampleValue} \times {Green}_{MaxSampleValue})
$$

In summary, the answer is based on finding the maximum count values for each
color per game.
This gives us an idea as to the minimum number of cubes for each color that were
in the bag for each game.

Once we have those values per game, then it's a simple matter of finding the
product across the values for each game and then the sum for all games.

## Solution

My solution from Part 1 already takes care of a large portion of the necessary
logic by mapping the data by finding the maximums for each color across a game.

In this part, we just need to loop through the parsed data without filtering to
find the products for each game and add those values together for the
sum product.
